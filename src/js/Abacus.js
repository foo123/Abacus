...
{
    K = null == K ? null : K|0; M = null == M ? null : M|0;
    var mul = Abacus.Arithmetic.mul, key, k;
    if ( (0 > n) || (K && M && ((K+M > n+1) || (K*M < n))) || (M && M > n) || (K && K > n) ) return Abacus.Arithmetic.O;
    ...
    if ( null == compositions.mem[key] )
    {
        if ( K && M )
        {
            ...
            // TODO
            compositions.mem[key] = Abacus.Arithmetic.O;
        }
        else if ( K )
        {
...
function intersection( common, a, b, dir, a0, a1, b0, b1 )
{
    dir = -1 === dir ? -1 : 1;
    if ( null == a0 ) a0 = 0;
    if ( null == a1 ) a1 = a.length-1;
    ...
    var ak = a0 > a1 ? -1 : 1, bk = b0 > b1 ? -1 : 1,
        ...
        ai = a0, bi = b0, il = 0;
    if ( null === common ) common = new Array(stdMath.min(al,bl));
    if ( 0 === common.length ) return common;
    ...
    // truncate if needed
    if ( il < common.length ) common.length = il;
    return common;
}
...
function merge( union, a, b, dir, a0, a1, b0, b1, unique, inplace )
{
    dir = -1 === dir ? -1 : 1;
    if ( null == a0 ) a0 = 0;
    if ( null == a1 ) a1 = a.length-1;
...
function mergesort( a, dir, a0, a1 )
{
    if ( null == a0 ) a0 = 0;
    if ( null == a1 ) a1 = a.length-1;
    ...
    while( logN )
    {
        n = N-size;
        for(j=0; j<n; j+=size2)
            merge(aux, a, a, dir, a0+ak*j, a0+ak*(j+size-1), a0+ak*(j+size), a0+ak*min(j+size2-1, N-1), false, true);
...
function multiset( m, n, dir )
{
    var nm = m ? m.length : 0, dk = 1, k = 0,
        k, ki = 0, mk = ki < nm ? m[ki]||1 : 1;
    if ( -1 === dir ){ dk = -1; k = nm-1; }
    return array(n||nm, function(){
        if ( 0 >= mk )
        {
            ki++; k+=dk;
            mk = ki<nm ? m[ki]||1 : 1;
...
function multipermutation2inversion( permutation, multiset, dir, inversion )
{
    ...
    for(var i=n-1; i>0; i--)
    {
        ...
        inversion[i] = binarysearch(permutation[n-1-i], multiset, 1);
        multiset.splice(inversion[i], 1);
    }
    inversion[0] = 0;
    return inversion;
}
function permutation2inversion( permutation, N, dir, inversion )
{
    // O(n log n) inversion computation
...
function inversion2permutation( inversion, N, dir, permutation )
{
    // O(n log n) inversion computation
    // "Efficient Algorithms to Rank and Unrank Permutations in Lexicographic Order", Blai Bonet (http://ldc.usb.ve/~bonet/reports/AAAI08-ws10-ranking.pdf)
...
{
    if ( M ) return null;
    if ( null == item_ ) item_ = composition2subset(item, 1);
    ...
    return subset2composition(item_, 1);
}
...
Permutation = Abacus.Permutation = Class(CombinatorialIterator, {
    ...
    ,__static__: {
        ...
        ,initial: function( n, $, dir ) {
            ...
            {
                item = multiset($.multiplicity, n, 0>dir?-1:1);
            }
            else if ( "derangement" === type )
            {
                if ( 2 > n ) return null;
...
Partition = Abacus.Partition = Class(CombinatorialIterator, {
    ...
    ,__static__: {
        ...
        ,count: function( n, $ ) {
            ...
            return ("composition" === type)||("set" === type) ? Abacus.Math.compositions(n, K, M) : Abacus.Math.partitions(n, K, M);
        }
        ,initial: function( n, $, dir ) {
            // some C-P-T dualities, symmetries & processes at play here
            // last (0>dir) is C-symmetric of first (0<dir)
            var klass = this, item, k, m,
                ...
                K = $ && $["parts="] ? $["parts="]|0 : null,
                order = $ && null!=$.order ? $.order : LEX,
                is_composition = ("composition" === type) || ("set" === type);
        ...
        ,output: function( item, index, n, $ ) {
            ...
            if ( ("composition" === type) || ("set" === type) )
                return "set" === type ? partition2sets(item) : item.slice();
        ...
        ,subset: function( item, dir, type ) {
            ...
            if ( "partition" === type ) mergesort(item, dir);
            return item;
        }
    ...
    ,item_: function( item, item_ ) {
        var type = this.$.type;
        if ( null == item || ("composition" !== type && "set" !== type) ) return null;
        return composition2subset(item, 1);
    }
});
// aliases
