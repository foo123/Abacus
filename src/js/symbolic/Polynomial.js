...
var UniPolyTerm = Class({
    ...
    ,toTerm: function(symbol, asTex, monomialOnly, asDec, precision) {
        ...
        if (true !== monomialOnly)
        {
            if (true === asDec)
            {
                term = term.length ? ((c.equ(I) ? '' : (c.equ(J) ? '-' : (is_instance(c, [RationalFunc, MultiPolynomialMod]) && !c.isConst() ? ('(' + c.toDec(precision) + ')') : (!c.isReal() ? ('(' + c.toDec(precision) + ')') : c.toDec(precision))))) + term) : (is_instance(c, [RationalFunc, MultiPolynomialMod]) && !c.isConst() ? '(' + c.toDec(precision) + ')' : c.toDec(precision));
            }
            else if (true === asTex)
            {
                term = term.length ? ((c.equ(I) ? '' : (c.equ(J) ? '-' : (is_instance(c, [RationalFunc, MultiPolynomialMod]) && !c.isConst() ? ('(' + c.toTex() + ')') : (!c.isReal() ? ('(' + c.toTex() + ')') : c.toTex())))) + term) : (is_instance(c, [RationalFunc, MultiPolynomialMod]) && !c.isConst() ? '(' + c.toTex() + ')' : c.toTex());
            }
            else
            {
                term = term.length ? ((c.equ(I) ? '' : (c.equ(J) ? '-' : (is_instance(c, [RationalFunc, MultiPolynomialMod]) && (!c.isConst() || !c.den.equ(Arithmetic.I)) ? ('(' + c.toString() + ')*') : (!c.isReal() ? ('(' + c.toString() + ')*') : (c.toString(true) + '*'))))) + term) : (is_instance(c, [RationalFunc, MultiPolynomialMod]) && !c.isConst() ? '(' + c.toString() + ')' : c.toString());
            }
        }
        return term;
    }
...
Polynomial = Abacus.Polynomial = Class(Poly, {
    ...
    constructor: function Polynomial(terms, symbol, ring) {
        ...
        if (is_instance(terms, [MultiPolynomialMod, MultiPolynomial]))
        {
    ...
    ,primitive: function(and_content) {
        ...
        if (null == self._prim)
        {
            ...
            else if (is_class(ring.PolynomialClass, [RationalFunc, MultiPolynomialMod]))
            {
                ...
                self._prim = [Polynomial(coeffp.map(function(c, i) {return UniPolyTerm(c, terms[i].e, ring);}), symbol, ring), field.create(content).div(field.create(LCM))];
            }
    ...
    ,resultant: function(other) {
        return is_instance(other, [MultiPolynomial, MultiPolynomialMod]) ? other[CLASS].Resultant(this, other, this.symbol) : Polynomial.Resultant(this, other);
    }
    ...
    ,equ: function(other) {
        ...
        if (is_instance(other, [MultiPolynomial, MultiPolynomialMod, RationalFunc]))
        {
    ...
    ,gt: function(other) {
        ...
        if (is_instance(other, [RationalFunc, MultiPolynomialMod, MultiPolynomial]))
        {
    ...
    ,add: function(other) {
        ...
        else if (is_instance(other, [RationalFunc, MultiPolynomialMod, MultiPolynomial]))
        {
    ...
    ,_div: function(other, q_and_r) {
        return uni_div(this, other, true, true === q_and_r);
    }
    ,div: function(other, q_and_r) {
        ...
        else if (is_instance(other, RationalFunc))
        {
            ...
            return q_and_r ? [ab.a.div(ab.b), Polynomial.Zero(ab.a.symbol, ab.a.ring)] : ab.a.div(ab.b);
        }
        else if (is_instance(other, MultiPolynomialMod))
        {
            if (!is_same_symbol(self.symbol, other.symbol))
            {
                ab = convert_to_same_symbol(self, other.p);
            }
            else
            {
                ab = {a:MultiPolynomial(self, other.symbol, self.ring), b:other.p};
            }
            return MultiPolynomialMod(ab.a, other.m).div(MultiPolynomialMod(ab.b, other.m), q_and_r, q_and_r);
        }
        ...
        if (is_instance(other, [Polynomial, Numeric]) || Abacus.Arithmetic.isNumber(other))
        {
            return uni_div(self, other, true, q_and_r);
        }
        return q_and_r ? [self, Polynomial.Zero(self.symbol, self.ring)] : self;
    }
    ,multidiv: function(others, q_and_r) {
        ...
        return poly_multi_divmod(self, others, true, q_and_r);
    }
    ,_mod: function(other) {
        return uni_div(this, other, false, true);
    }
    ,mod: function(other) {
        var self = this, ab;
        if (self.ring.contains(other)) other = Polynomial.Const(other, self.symbol, self.ring);
        ...
        else if (is_instance(other, MultiPolynomialMod))
        {
            ...
            {
                ab = {a:RationalFunc(MultiPolynomial(self, other.symbol, self.ring)), b:other.p};
            }
            return MultiPolynomialMod(ab.a, other.m).mod(ab.b);
        }
        else if (is_instance(other, Poly) && !is_same_symbol(self.symbol, other.symbol))
        {
            ab = convert_to_same_symbol(self, other);
            return ab.a.mod(ab.b);
        }
        return is_instance(other, [Polynomial, Numeric]) || Abacus.Arithmetic.isNumber(other) ? uni_div(self, other, false, true) : Polynomial.Zero(self.symbol, self.ring);
    }
    ,multimod: function(others) {
        var self = this;

        if (is_instance(others, Polynomial)) others = [others];
        if (!others || !others.length) return self;

        others = others.map(function(q) {return self.ring.contains(q) ? Polynomial.Const(q, self.symbol, self.ring) : q;});
        return poly_multi_divmod(self, others, false, true);
    }
    ...
    ,toDec: function(precision) {
        ...
        {
            ti = t[i];
            out += (prev && ((is_instance(ti.c, [RationalFunc, MultiPolynomialMod]) && (!ti.c.isConst() || !ti.c.den.equ(Arithmetic.I))) || !ti.c.isReal() || ti.c.gt(Arithmetic.O)) ? '+' : '') + ti.toTerm(x, false, false, true, precision);
            prev = true;
        }
        if (!out.length)
        {
...
var MultiPolyTerm = Class({
    ...
    constructor: function MultiPolyTerm(c, e, ring, order) {
        ...
        if (is_instance(self.c, [MultiPolynomial, MultiPolynomialMod, RationalFunc])) self.c = self.c.order(order);
    }
    ...
    ,toTerm: function(symbol, asTex, monomialOnly, asDec, precision) {
        ...
        if (true !== monomialOnly)
        {
            if (true === asDec)
            {
                term = term.length ? ((c.equ(I) ? '' : (c.equ(J) ? '-' : (is_instance(c, [MultiPolynomial, MultiPolynomialMod, RationalFunc]) && !c.isConst() ? ('(' + c.toDec(precision) + ')') : (!c.isReal() ? ('(' + c.toDec(precision) + ')') : c.toDec(precision))))) + term) : (is_instance(c, [MultiPolynomial, MultiPolynomialMod, RationalFunc]) && !c.isConst() ? '(' + c.toDec(precision) + ')' : c.toDec(precision));
            }
            else if (true === asTex)
            {
                term = term.length ? ((c.equ(I) ? '' : (c.equ(J) ? '-' : (is_instance(c, [MultiPolynomial, MultiPolynomialMod, RationalFunc]) && !c.isConst() ? ('(' + c.toTex() + ')') : (!c.isReal() ? ('(' + c.toTex() + ')') : c.toTex())))) + term) : (is_instance(c, [MultiPolynomial, MultiPolynomialMod, RationalFunc]) && !c.isConst() ? '(' + c.toTex() + ')' : c.toTex());
            }
            else
            {
                term = term.length ? ((c.equ(I) ? '' : (c.equ(J) ? '-' : ((is_instance(c, [MultiPolynomial, MultiPolynomialMod]) && !c.isConst()) || (is_instance(c, RationalFunc) && (!c.isConst() || !c.den.equ(Arithmetic.I))) ? ('(' + c.toString() + ')*') : (!c.isReal() ? ('(' + c.toString() + ')*') : (c.toString(true) + '*'))))) + term) : ((is_instance(c, [MultiPolynomial, MultiPolynomialMod]) && !c.isConst()) || (is_instance(c, RationalFunc) && (!c.isConst() || !c.den.equ(Arithmetic.I))) ? '(' + c.toString() + ')' : c.toString());
            }
        }
        return term;
    }
...
MultiPolynomial = Abacus.MultiPolynomial = Class(Poly, {
    ...
    constructor: function MultiPolynomial(terms, symbol, ring) {
        ...
        if (is_instance(terms, [MultiPolynomial, MultiPolynomialMod]))
        {
            self.ring = ring || terms.ring;
    ...
    ,isMono: function() {
        // is monomial
        var terms = this.terms;
        return (1 === terms.length) && ((!is_instance(terms[0].c, [MultiPolynomial, MultiPolynomialMod, RationalFunc]) || terms[0].c.isMono()) && 0 !== cmp_exp_i(terms[0].e, [0]));
    }
    ,isConst: function() {
        var terms = this.terms;
        return (0 === terms.length) || ((1 === terms.length) && (!is_instance(terms[0].c, [MultiPolynomial, MultiPolynomialMod, RationalFunc]) || terms[0].c.isConst()) && (0 === cmp_exp_i(terms[0].e, [0])));
    }
    ,isUni: function(x, strict) {
    ...
    ,primitive: function(and_content) {
        ...
        if (null == self._prim)
        {
            ...
            else if (is_class(ring.PolynomialClass, [MultiPolynomial, MultiPolynomialMod, RationalFunc]))
            {
                LCM = terms.reduce(function(LCM, t) {return t.c.den.mul(LCM);}, ring.One().num);
                coeffp = terms.map(function(t) {return t.c.mul(LCM).num;});
                content = LCM[CLASS].gcd(coeffp);
                ...
                self._prim = [MultiPolynomial(coeffp.map(function(c, i) {return MultiPolyTerm(c, terms[i].e, ring, terms[i].order);}), symbol, ring), field.create(content).div(field.create(LCM))];
            }
            else if (is_class(ring.NumberClass, Complex))
            {
                isReal = self.isReal(); isImag = self.isImag();
    ...
    ,homogeneous: function(z) {
        ...
        if (self.terms.length)
        {
            ...
            return MultiPolynomial(self.terms.map(function(term) {
                var d = degree;
                return MultiPolyTerm(term.c, array(symbol.length, function(i) {
                    if (i < term.e.length)
                    {
                        d -= term.e[i];
                        return term.e[i];
    ...
    ,add: function(other) {
        ...
        else if (is_instance(other, [RationalFunc, MultiPolynomialMod]))
        {
            return other.add(self);
        }
        else if (is_instance(other, Poly) && !is_same_symbol(self.symbol, other.symbol))
    ...
    ,_div: function(other, q_and_r) {
        return multi_div(this, other, true, true === q_and_r);
    }
    ,div: function(other, q_and_r) {
        var self = this, ab;
        q_and_r = (true === q_and_r);
        ...
        {
            return q_and_r ? [RationalFunc(self).div(other), MultiPolynomial.Zero(self.symbol, self.ring)]: RationalFunc(self).div(other);
        }
        else if (is_instance(other, MultiPolynomialMod))
        {
            return MultiPolynomialMod(self, other.m).div(other, q_and_r, q_and_r);
        }
        else if (is_instance(other, Poly) && !is_same_symbol(self.symbol, other.symbol))
        {
            ab = convert_to_same_symbol(self, other);
        ...
        return q_and_r ? [self, MultiPolynomial.Zero(self.symbol, self.ring)] : self;
    }
    ,multidiv: function(others, q_and_r) {
        var self = this;
    ...
    ,_mod: function(other) {
        return multi_div(this, other, false, true);
    }
    ,mod: function(other) {
        var self = this, ab;
        if (self.ring.contains(other)) other = MultiPolynomial.Const(other, self.symbol, self.ring);
        ...
        {
            return MultiPolynomialMod(self, other.m).mod(other);
        }
        else if (is_instance(other, Poly) && !is_same_symbol(self.symbol, other.symbol))
        {
            ab = convert_to_same_symbol(self, other);
            return multi_div(ab.a, ab.b, false, true);
        }
        else if (is_instance(other, [Numeric, Poly]) || Abacus.Arithmetic.isNumber(other))
        {
            return multi_div(self, other, false, true);
        }
        return MultiPolynomial.Zero(self.symbol, self.ring);
    }
    ,multimod: function(others) {
        var self = this;
    ...
    ,shift: function(x, s) {
        ...
        if (-1 === index)
        {
            if (is_class(ring.PolynomialClass, [MultiPolynomial, MultiPolynomialMod, RationalFunc]))
            {
                return MultiPolynomial(self.terms.map(function(term) {
    ...
    ,evaluate: function(x, all) {
        ...
        function get_val(p, x)
        {
            ...
            {
                if (is_instance(p, [MultiPolynomial, MultiPolynomialMod, RationalFunc]))
                {
                    if (is_instance(p, MultiPolynomialMod))
                    {
                        p = p.p;
                    }
    ...
    ,toDec: function(precision) {
        ...
        {
            ti = t[i];
            out += (prev && (((is_instance(ti.c, [MultiPolynomial, MultiPolynomialMod]) && !ti.c.isConst()) || (is_instance(ti.c, RationalFunc) && (!ti.c.isConst() || !ti.c.den.equ(Arithmetic.I)))) || !ti.c.isReal() || ti.c.gt(Arithmetic.O)) ? '+' : '') + ti.toTerm(x, false, false, true, precision);
            prev = true;
        }
        if (!out.length)
        {
...
// Abacus.MultiPolynomialMod, represents a multivariate polynomial modulo other polynomials
MultiPolynomialMod = Abacus.MultiPolynomialMod = Class(Poly, {

    constructor: function MultiPolynomialMod(poly, mod) {
        var self = this;
        if (!is_instance(self, MultiPolynomialMod)) return new MultiPolynomialMod(poly, mod);

        if (is_instance(poly, MultiPolynomialMod))
        {
            self.p = poly.p.clone();
            self.m = mod || (poly.m.slice());
        }
        else
        {
            self.p = poly;
        ...
        def(self, 'terms', {
            get: function() {
                return self.p ? self.p.terms : null;
            },
            set: NOP,
            enumerable: true,
            configurable: false
        ...
        def(self, 'symbol', {
            get: function() {
                return self.p ? self.p.symbol : null;
            },
            set: NOP,
            enumerable: true,
            configurable: false
        ...
        def(self, 'ring', {
            get: function() {
                return self.p ? self.p.ring : null;
            },
            set: NOP,
            enumerable: true,
            configurable: false
        ...
        def(self, 'den', {
            get: function() {
                return MultiPolynomialMod.One(self.symbol, self.ring, self.m);
            },
            set: NOP,
            enumerable: true,
            configurable: false
    ...
    ,__static__: {
        ...
        ,Zero: function(symbol, ring, modulo) {
            return new MultiPolynomialMod(new MultiPolynomial([], symbol || ['x'], ring || Ring.Q()), modulo);
        }
        ,One: function(symbol, ring, modulo) {
            ring = ring || Ring.Q();
            return new MultiPolynomialMod(new MultiPolynomial({'1':ring.One()}, symbol || ['x'], ring), modulo);
        }
        ,MinusOne: function(symbol, ring, modulo) {
            ring = ring || Ring.Q();
            return new MultiPolynomialMod(new MultiPolynomial({'1':ring.MinusOne()}, symbol || ['x'], ring), modulo);
        }
        ,Const: function(c, symbol, ring, modulo) {
            return new MultiPolynomialMod(new MultiPolynomial({'1':c || Abacus.Arithmetic.O}, symbol || ['x'], ring || Ring.Q()), modulo);
        }

        ,cast: null // added below

        ,fromString: function(s, symbol, ring, modulo) {
            return MultiPolynomialMod.fromExpr(Expr.fromString(s, Complex.Symbol), symbol, ring, modulo);
        }
        ,fromExpr: function(e, symbol, ring, modulo) {
            ...
            symbol = symbol || ['x'];
            if (!is_array(symbol)) symbol = [String(symbol || 'x')];
            return new MultiPolynomialMod(poly_mod(e.toPoly(symbol, ring, Complex.Symbol), modulo), modulo);
        }
    }
    ...
    ,isReal: function() {
        return this.p.isReal();
    }
    ,isImag: function() {
        return this.p.isImag();
    }
    ,isComplex: function() {
        return this.p.isComplex();
    }
    ,isMono: function() {
        return this.p.isMono();
    }
    ,isConst: function() {
        return this.p.isConst();
    }
    ,isUni: function(x, strict) {
        return this.p.isUni(x, strict);
    }
    ,deg: function(x) {
        return this.p.deg(x);
    }
    ,maxdeg: function(x) {
        return this.p.maxdeg(x);
    }
    ,mindeg: function(x) {
        return this.p.mindeg(x);
    }
    ,term: function(i, as_degree, term_only) {
        return this.p.term(i, as_degree, term_only);
    }
    ,ltm: function(asPoly, x) {
        return this.p.ltm(asPoly, x);
    }
    ,ttm: function(asPoly, x) {
        return this.p.ttm(asPoly, x);
    }
    ,lm: function(x) {
        return this.p.lm(x);
    }
    ...
    ,order: function(order) {
        var self = this;
        if (!arguments.length) return self.p.order();
        return MultiPolynomialMod(self.p.order(order), self.m);
    }
    ,monic: function(with_factor) {
        var self = this, monic = self.p.monic(with_factor);
        return with_factor ? [MultiPolynomialMod(monic[0], self.m), monic[1]] : MultiPolynomialMod(monic, self.m);
    }
    ,primitive: function(and_content) {
        var self = this, primitive = self.p.primitive(and_content);
        return and_content ? [MultiPolynomialMod(primitive[0], self.m), primitive[1]] : MultiPolynomialMod(primitive, self.m);
    }
    ,content: function() {
        return this.p.content();
    }
    ,homogeneous: function(z) {
        return MultiPolynomialMod(this.p.homogeneous(z), this.m);
    }
    ,resultant: function(other, x) {
        return MultiPolynomial.Resultant(this, other, x);
    }
    ...
    ,factors: function() {
        var self = this;
        return [self.p.factors()[0].map(function(p) {
            return [MultiPolynomialMod(p[0], self.m), p[1]];
        }), self.p.factors()[1]];
    }
    ,equ: function(other) {
        return this.p.equ(other.p || other);
    }
    ,gt: function(other) {
        return this.p.gt(other.p || other);
    }
    ,gte: function(other) {
        return this.p.gte(other.p || other);
    }
    ,lt: function(other) {
        return this.p.lt(other.p || other);
    }
    ,lte: function(other) {
        return this.p.lte(other.p || other);
    }

    ,real: function() {
        return MultiPolynomialMod(this.p.real(), this.m);
    }
    ,imag: function() {
        return MultiPolynomialMod(this.p.imag(), this.m);
    }
    ,abs: function() {
        return MultiPolynomialMod(this.p.abs(), this.m);
    }
    ,conj: function() {
        var self = this;
        if (null == self._c)
        {
            self._c = MultiPolynomialMod(self.p.conj(), self.m);
            self._c._c = self;
        }
        return self._c;
    }
    ,neg: function() {
        ...
        if (null == self._n)
        {
            self._n = MultiPolynomialMod(self.p.neg(), self.m);
            self._n._n = self;
        }
        return self._n;
    }
    ,inv: function() {
        ...
        if (null == self._i)
        {
            if (self.p.equ(0))
            {
                throw new Error('Inverse of Zero in Abacus.MultiPolynomialMod!');
            }
            else
            {
                // compute multiplicative inverse
                // via extended euclidean algorithm recursively
                poly = self.p;
                modulo = self.m;
                inv = MultiPolynomial.One(poly.symbol, poly.ring);
                n = modulo.length;
                for (i=0; i<n; ++i)
                {
                    // disable GCD simplification in RF
                    // because it creates combinatorial explosion and long/infinite loops
                    // by not taking into account the modulo simplifications
                    tmp = RationalFunc.__MAXGCD__;
                    RationalFunc.__MAXGCD__ = 0;
                    d = MultiPolynomial.xgcd(poly, modulo[i]);
                    RationalFunc.__MAXGCD__ = tmp;
                    inv = poly_mod(d[1].num.mul(inv), modulo);
                    poly = poly_mod(d[1].den.mul(d[0]), modulo);
                }
                d = poly_mod(inv.mul(self.p), self.m).c();
                self._i = MultiPolynomialMod(d.equ(0) ? inv : inv.div(d), self.m);
                self._i._i = self;
            }
        }
        return self._i;
    ...
    ,_add: function(other) {
        var self = this;
        return MultiPolynomialMod(poly_mod(self.p._add(other.p || other), self.m), self.m);
    }
    ,add: function(other) {
        var self = this;
        if (is_instance(other, Expr))
        ...
        return MultiPolynomialMod(poly_mod(self.p.add(other.p || other), self.m), self.m);
    }
    ,_sub: function(other) {
        var self = this;
        return MultiPolynomialMod(poly_mod(self.p._sub(other.p || other), self.m), self.m);
    }
    ,sub: function(other) {
        var self = this;
        if (is_instance(other, Expr))
        ...
        return MultiPolynomialMod(poly_mod(self.p.sub(other.p || other), self.m), self.m);
    }
    ,_mul: function(other) {
        var self = this;
        return MultiPolynomialMod(poly_mod(self.p._mul(other.p || other), self.m), self.m);
    }
    ,mul: function(other) {
        var self = this;
        if (is_instance(other, Expr))
        ...
        return MultiPolynomialMod(poly_mod(self.p.mul(other.p || other), self.m), self.m);
    }
    ,_div: function(other, q_and_r, poly_div) {
        var self = this, qr, inv;
        q_and_r = (true === q_and_r);
        if (true === poly_div)
        {
            qr = self.p._div(other.p || other, q_and_r);
            return q_and_r ? [MultiPolynomialMod(poly_mod(qr[0], self.m), self.m), MultiPolynomialMod(poly_mod(qr[1], self.m), self.m)] : MultiPolynomialMod(qr, self.m);
        }
        else if (is_instance(other, Numeric) || Abacus.Arithmetic.isNumber(other))
        {
            return q_and_r ? [MultiPolynomialMod(poly_mod(self.p._div(other), self.m), self.m), MultiPolynomialMod.Zero(self.symbol, self.ring, self.m)] : MultiPolynomialMod(poly_mod(self.p._div(other), self.m), self.m);
        }
        else
        {
            inv = (other.p && modulo_match(self.m, other.m) ? other : MultiPolynomialMod(MultiPolynomial(other.p || other, self.symbol, self.ring), self.m)).inv();
            return q_and_r ? [self._mul(inv), MultiPolynomialMod.Zero(self.symbol, self.ring, self.m)] : self._mul(inv);
        }
    }
    ,div: function(other, q_and_r, poly_div) {
        var self = this, qr, inv;
        q_and_r = (true === q_and_r);
        if (is_instance(other, Expr))
        {
        ...
        if (true === poly_div)
        {
            qr = self.p.div(other.p || other, q_and_r);
            return q_and_r ? [MultiPolynomialMod(poly_mod(qr[0], self.m), self.m), MultiPolynomialMod(poly_mod(qr[1], self.m), self.m)] : MultiPolynomialMod(qr, self.m);
        }
        else if (is_instance(other, Numeric) || Abacus.Arithmetic.isNumber(other))
        {
            return q_and_r ? [MultiPolynomialMod(poly_mod(self.p.div(other), self.m), self.m), MultiPolynomialMod.Zero(self.symbol, self.ring, self.m)] : MultiPolynomialMod(poly_mod(self.p.div(other), self.m), self.m);
        }
        else
        {
            inv = (other.p && modulo_match(self.m, other.m) ? other : MultiPolynomialMod(MultiPolynomial(other.p || other, self.symbol, self.ring), self.m)).inv();
            return q_and_r ? [self.mul(inv), MultiPolynomialMod.Zero(self.symbol, self.ring, self.m)] : self.mul(inv);
        }
    }
    ,multidiv: function(others, q_and_r) {
        var self = this, qr = self.p.multidiv([].concat(others).map(function(p) {return p.p || p;}), q_and_r);
        return q_and_r ? [qr[0].map(function(q) {return MultiPolynomialMod(poly_mod(q, self.m), self.m);}), MultiPolynomialMod(poly_mod(qr[1], self.m), self.m)] : qr.map(function(q) {return MultiPolynomialMod(poly_mod(q, self.m), self.m);});
    }
    ,_mod: function(other) {
        var self = this;
        return MultiPolynomialMod(self.p._mod(other.p || other), self.m);
    }
    ,mod: function(other) {
        var self = this;
        if (is_instance(other, Expr))
        ...
        return MultiPolynomialMod(self.p.mod(other.p || other), self.m);
    }
    ,multimod: function(others) {
        var self = this;
        return MultiPolynomialMod(poly_mod(self.p.multimod([].concat(others).map(function(p) {return p.p || p;})), self.m), self.m);
    }
    ,_divmod: function(other) {
        var self = this, qr = self.p._divmod(other.p || other);
        return [MultiPolynomialMod(poly_mod(qr[0], self.m), self.m), MultiPolynomialMod(poly_mod(qr[1], self.m), self.m)];
    }
    ,divmod: function(other) {
        var self = this, qr = self.p.divmod(other.p || other);
        return [MultiPolynomialMod(poly_mod(qr[0], self.m), self.m), MultiPolynomialMod(poly_mod(qr[1], self.m), self.m)];
    }
    ,multidivmod: function(others) {
        var self = this, qr = self.p.multidivmod([].concat(others).map(function(p) {return p.p || p;}));
        return [qr[0].map(function(q) {return MultiPolynomialMod(poly_mod(q, self.m), self.m);}), MultiPolynomialMod(poly_mod(qr[1], self.m), self.m)];
    }
    ,divides: function(other) {
        return !this.p.equ(Abacus.Arithmetic.O);
    }
    ,_pow: function(n) {
        var self = this;
        return MultiPolynomialMod(poly_mod(self.p._pow(n), self.m), self.m);
    }
    ,pow: function(n) {
        var self = this;
        return MultiPolynomialMod(poly_mod(self.p.pow(n), self.m), self.m);
    }
    ,rad: function(n) {
        var self = this;
        return MultiPolynomialMod(poly_mod(self.p.rad(n), self.m), self.m);
    }
    ,substitute: function(v, xi) {
        var self = this;
        return MultiPolynomialMod(poly_mod(self.p.substitute(v, xi), self.m), self.m);
    }
    ,compose: function(q) {
        var self = this;
        return MultiPolynomialMod(poly_mod(self.p.compose(q), self.m), self.m);
    }
    ,shift: function(x, s) {
        var self = this;
        return MultiPolynomialMod(poly_mod(self.p.shift(x, s), self.m), self.m);
    }
    ,d: function(x, n) {
        var self = this;
        return MultiPolynomialMod(poly_mod(self.p.d(x, n), self.m), self.m);
    }
    ,evaluate: function(x, all) {
        return this.p.evaluate(x, all);
    }
    ,univariate: function(x, as_field) {
        return MultiPolynomialMod(this.p.univariate(x, as_field), this.m);
    }
    ,multivariate: function(x) {
        return MultiPolynomialMod(this.p.multivariate(x), this.m);
    }
    ,toExpr: function() {
        return this.p.toExpr();
    }
    ,toString: function() {
    ...
    ,toDec: function(precision) {
        return this.p.toDec(precision);
    }
});
MultiPolynomialMod.cast = function(a, symbol, ring, modulo) {
    ring = ring || Ring.Q();
    symbol = symbol || 'x';
    if (!is_array(symbol)) symbol = [String(symbol)];
    var type_cast = typecast(function(a) {
        return is_instance(a, MultiPolynomialMod) && is_same_symbol(symbol, a.symbol) && modulo_match(modulo, a.m) && ring.equ(a.ring);
    }, function(a) {
        return is_string(a) ? MultiPolynomialMod.fromString(a, symbol, ring, modulo) : new MultiPolynomialMod(poly_mod(MultiPolynomial.cast(a, symbol, ring), modulo), modulo);
    });
    return type_cast(a);
};
...
function monomial_total_deg(e)
{
    return e.reduce(function(sum, e, i) {return sum + e;}, 0);
}
function cmp_monomial_total_deg(e1, e2)
{
    return monomial_total_deg(e1) - monomial_total_deg(e2);
}
...
function division_sparse(a, b, TermClass, want_q, want_r, ring)
{
    ...
    if (!b.length || (!want_q && !want_r)) return null;
    TermClass = TermClass === MultiPolyTerm ? MultiPolyTerm : UniPolyTerm;
    ring = ring || Ring.Q();
    ...
    while ((d=heap.peek()) || (k < na))
    {
        ...
        {
            Q = res.div(b0);
            if (want_q) q = addition_sparse(q, [Q], TermClass, false, ring);
            if (nb > 1) heap.push({term:Q.mul(b[1].cast(ring)), n:2, Q:Q});
        }
        else if (want_r)
        {
    ...
    return want_q && want_r ? [q, r] : (want_q || want_r ? (q || r) : null);
}
...
function uni_div(P, x, want_q, want_r)
{
    if (!want_q && !want_r) return null;
    var Arithmetic = Abacus.Arithmetic, O = Arithmetic.O, I = Arithmetic.I,
        symbol, q = null, r = null/*, d, diff, diff0*/;
    ...
    if (is_instance(x, Polynomial))
    {
        ...
        if (x.isConst())
        {
            ...
            x = x.cc();
            if (want_q) q = x.equ(I) ? P : Polynomial(array(P.terms.length, function(i) {
                return P.terms[i].div(x);
            }), P.symbol, P.ring);
            if (want_r) r = Polynomial.Zero(P.symbol, P.ring);
            return want_q && want_r ? [q, r] : (want_q || want_r ? (q || r) : null);
        }
        ...
        {
            // sparse polynomial reduction/long division
            q = division_sparse(P.terms, x.terms, UniPolyTerm, want_q, want_r, P.ring);
            return want_q && want_r ? [Polynomial(q[0], P.symbol, P.ring), Polynomial(q[1], P.symbol, P.ring)] : (want_q || want_r ? Polynomial(q, P.symbol, P.ring) : null);
        }
        else
        {
            // upgrade to multivariate polynomial
            symbol = P.symbol > x.symbol ? [x.symbol, P.symbol] : [P.symbol, x.symbol];
            return multi_div(MultiPolynomial(P, symbol, P.ring), MultiPolynomial(x, symbol, x.ring), want_q, want_r);
        }
    }
    else if (is_instance(x, Numeric) || Arithmetic.isNumber(x))
    {
...
function multi_div(P, x, want_q, want_r)
{
    if (!want_q && !want_r) return null;
    var Arithmetic = Abacus.Arithmetic, O = Arithmetic.O, I = Arithmetic.I,
        q = null, r = null/*, d, diff, diff0*/;
    ...
    if (is_instance(x, MultiPolynomial))
    {
        ...
        if (x.isConst())
        {
            // constant polynomial, simple numeric division
            x = x.cc();
            if (want_q) q = x.equ(I) ? P : MultiPolynomial(array(P.terms.length, function(i) {
                return P.terms[i].div(x);
            }), P.symbol, P.ring);
            if (want_r) r = MultiPolynomial.Zero(P.symbol, P.ring);
            return want_q && want_r ? [q, r] : (want_q || want_r ? (q || r) : null);
        }
        // sparse polynomial reduction/long division
        P = ensure_same_monomial_order(P, x);
        q = division_sparse(P.terms, x.terms, MultiPolyTerm, want_q, want_r, P.ring);
        return want_q && want_r ? [MultiPolynomial(q[0], P.symbol, P.ring), MultiPolynomial(q[1], P.symbol, P.ring)] : (want_q || want_r ? MultiPolynomial(q, P.symbol, P.ring) : null);
    }
    else if (is_instance(x, Numeric) || Arithmetic.isNumber(x))
    {
        /*if (Arithmetic.isNumber(x))*/ x = P.ring.cast(x);
...
function poly_multi_divmod(self, others, want_q, want_r)
{
    if (!want_q && !want_r) return null;
    var i, n = others.length,
        Poly = self[CLASS],
        q = want_q ? array(n, function() {return [];}) : null,
        r = want_r ? [] : null,
        p = self.clone(),
        xlt, plt, divides, t,
        Arithmetic = Abacus.Arithmetic;
    while (p.terms.length/*!p.equ(Arithmetic.O)*/)
    {
        ...
        if (divides)
        {
            ...
            t = plt.div(xlt);
            if (want_q) q[i] = addition_sparse(q[i], [t], Poly.Term, false, p.ring);
            p.terms = addition_sparse(p.terms, others[i].terms.map(function(xt) {return xt.mul(t);}), Poly.Term, true, p.ring);
        }
        else
        {
            // None of them divided. Cancel and Move the leading term to r.
            p.terms.shift();
            if (want_r) r = addition_sparse(r, [plt], Poly.Term, false, p.ring);
        }
    }
    if (want_q)
    {
        q = q.map(function(qi) {
            qi = new Poly(qi, p.symbol, p.ring);
            return qi;
        });
    }
    if (want_r)
    {
        r = new Poly(r, p.symbol, p.ring);
    }
    return want_q && want_r ? [q, r] : (want_q || want_r ? (q || r) : null);
}
/*function poly_symbol(p, ref)
{
    var symbol = p.symbol.slice(), rest = [], ring = p.ring;
    while (ring && ring.PolynomialSymbol && (rest.length+symbol.length < ref.length))
    {
        rest = rest.concat(ring.PolynomialSymbol);
        ring = ring.CoefficientRing;
    }
    return symbol.concat(rest);
}*/
function poly_mod(poly, modulo)
{
    if (poly && modulo && modulo.length)
    {
        //for (var i=0,n=modulo.length; i<n; ++i) poly = poly.mod(modulo[i]);
        poly = poly_multi_divmod(poly, modulo, false, true);
    }
    return poly;
}
...
function polygcd(/* args */)
{
    ...
    var args = arguments.length && (is_array(arguments[0]) || is_args(arguments[0])) ? arguments[0] : arguments,
        c = args.length, Arithmetic = Abacus.Arithmetic, PolynomialClass = Polynomial, S, R, M, are_const = true,
    ...
    if (are_const) return PolynomialClass.Const(R.gcd(array(c, function(i) {return args[i].cc();})), S, R);
    ...
    if (/*is_class(R.NumberClass, Integer)*/!R.isField())
    {
        ...
        else //if (2 <= c)
        {
            ...
            p = new PolynomialClass(a, S, field);
            q = new PolynomialClass(2 === c ? args[1] : polygcd(slice.call(args, 1)), S, field);
            return new PolynomialClass(polygcd(p, q).primitive().mul(field.gcd(p.content(), q.content())), S, R);
        }
    }
    ...
    while (i < c)
    {
        ...
        while (!b.equ(O))
        {
            ...
            a = b;
            b = r;
            if (b.equ(a0) && a.equ(b0)) break; // will not change anymore
        }
    }
...
function polyxgcd(/* args */)
{
    ...
    var args = arguments.length && (is_array(arguments[0]) || is_args(arguments[0])) ? arguments[0] : arguments,
        ...
        PolynomialClass = Polynomial, S, R, M, are_const = true,
    ...
    if (are_const) return R.xgcd(array(k, function(i) {return args[i].cc();})).map(function(g) {return PolynomialClass.Const(g, S, R);});
    ...
    if (/*is_class(R.NumberClass, Integer)*/!R.isField())
    {
        ...
        else //if (2 <= k)
        {
            ...
            b = gcd[0];
            p = new PolynomialClass(a, S, field);
            q = new PolynomialClass(b, S, field);
            g = polyxgcd(p, q);
            ...
            return array(gcd.length+1, function(i) {
                return 0 === i ? new PolynomialClass(g[0], S, R) : (1 === i ? g[1] : gcd[i-1].mul(g[2]));
            });
        }
    }
...
function polylcm(/* args */)
{
    ...
    var args = arguments.length && (is_array(arguments[0]) || is_args(arguments[0])) ? arguments[0] : arguments,
        i, l = args.length, LCM, O = Abacus.Arithmetic.O, PolynomialClass = Polynomial, S, R, M;
    if (1 >= l) return 1 === l ? args[0] : PolynomialClass.Zero();
    PolynomialClass = args[0][CLASS];
    S = args[0].symbol;
    R = args[0].ring;
    if (1 >= l) return 1 === l ? args[0] : PolynomialClass.Zero(S, R);
...
function poly_pp_factor(f, k)
{
    ...
    else if (is_instance(f, [MultiPolynomial, MultiPolynomialMod]))
    {
...
MultiPolynomialMod.kthroot = MultiPolynomial.kthroot = Polynomial.kthroot = polykthroot;
...
MultiPolynomial.lcm = function(/*args*/) {
    // slow and consuming for more than 2 or 3 variables
    var args = arguments.length && (is_array(arguments[0]) || is_args(arguments[0])) ? arguments[0] : arguments,
        symbol = null, order, symbolord, lcm;
    // make sure the symbols present in all are put first,
    // else the gcd computation may produce 1 as gcd
    symbolord = args[0].symbol.map(function(xi, i) {
        return {x:xi, d:[].reduce.call(args, function(d, pi) {
...
MultiPolynomialMod.gcd = function(/*args*/) {
    var args = arguments.length && (is_array(arguments[0]) || is_args(arguments[0])) ? arguments[0] : arguments, M;
    if (args.length)
    {
        M = args[0].m;
        return MultiPolynomialMod(MultiPolynomial.gcd([].map.call(args, function(p) {return p.p || p;})), M);
    }
    return MultiPolynomialMod.Zero();
};
MultiPolynomialMod.xgcd = function(/*args*/) {
    var args = arguments.length && (is_array(arguments[0]) || is_args(arguments[0])) ? arguments[0] : arguments, M;
    if (args.length)
    {
        M = args[0].m;
        var gcd = MultiPolynomial.xgcd([].map.call(args, function(p) {return p.p || p;})), LCM = gcd.reduce(function(LCM, g, i) {
            return 0 === i ? LCM : g.den.mul(LCM);
        }, 1);
        return gcd.map(function(g, i) {
            return MultiPolynomialMod(poly_mod(0 === i ? (g.mul(LCM)) : (g.num.mul(LCM.div(g.den))), M), M);
        });
    }
};
MultiPolynomialMod.lcm = function(/*args*/) {
    var args = arguments.length && (is_array(arguments[0]) || is_args(arguments[0])) ? arguments[0] : arguments, M;
    if (args.length)
    {
        M = args[0].m;
        return MultiPolynomialMod(poly_mod(MultiPolynomial.lcm([].map.call(args, function(p) {return p.p || p;})), M), M);
    }
    return MultiPolynomialMod.Zero();
};
...
Abacus.Poly = function(expr, ring_or_symbol) {
    if (is_string(expr))
    {
        return is_instance(ring_or_symbol, Ring) ? ring_or_symbol.fromString(expr) : (Expr(expr).toPoly(is_string(ring_or_symbol) || is_array(ring_or_symbol) ? ring_or_symbol : "x"));
    }
    if (is_instance(expr, Expr))
    {
        return is_instance(ring_or_symbol, Ring) ? ring_or_symbol.fromExpr(expr) : (expr.toPoly(is_string(ring_or_symbol) || is_array(ring_or_symbol) ? ring_or_symbol : "x"));
    }
    return expr;
};
