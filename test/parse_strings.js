...
echo(ring.fromString("(1/2)*i*x^2+(1+(2/3)*i)*x").toString());
echo('ring.fromString("(3/2+(1/2)*i)*x+1+(2/3)*i")');
echo(ring.fromString("(3/2+(1/2)*i)*x+1+(2/3)*i").toString());
echo('ring.fromString("1+2*i*x")'); /* this is taken as one complex coefficient unfortunately */
...
echo(ring.fromString("1+((2)*i)*x").toString());
echo('ring.fromString("2i*x+1")'); /* this is workaround, parsed correctly, in fact .toString() methods output strings in this order so always parsed correctly */
echo(ring.fromString("2i*x+1").toString());
echo();

ring = Abacus.Ring.C("x","y","z");
echo('ring = Abacus.Ring.'+ring.toString());
echo('----');
echo('ring.fromString("1 - y*x^2 + 3*x*y")');
echo(ring.fromString("1 - y*x^2 + 3*x*y").toString());
echo('ring.fromString("1 - y*x")');
echo(ring.fromString("1 - y*x").toString());
echo('ring.fromString("1 - y+x")');
echo(ring.fromString("1 - y+x").toString());
echo('ring.fromString("1+x*y+x*y^2")');
echo(ring.fromString("1+x*y+x*y^2").toString());
echo('ring.fromString("1 - y*x")');
echo(ring.fromString("1 - y*x").toString());
echo('ring.fromString("1+x*y+x*y^2")');
echo(ring.fromString("1+x*y+x*y^2").toString());
echo('ring.fromString("x^2*y+x^2*y^2+x+y*x+2")');
echo(ring.fromString("x^2*y+x^2*y^2+x+y*x+2").toString());
echo('ring.fromString("x^2*y+x^2*y^2+x+y*x+2+z*y*x+z*y")');
echo(ring.fromString("x^2*y+x^2*y^2+x+y*x+2+z*y*x+z*y").toString());
echo('ring.fromString("(4+2i)*x^2*y+(2+2i)*x^2*y^2+2x+4y*x+6z*y*x+(2+4i)*z*y+2")');
echo(ring.fromString("(4+2i)*x^2*y+(2+2i)*x^2*y^2+2x+4y*x+6z*y*x+(2+4i)*z*y+2").toString());
echo();

pring = Abacus.Ring.C("x", "y"); ring = pring.associatedField();
echo('ring = Abacus.Ring.'+ring.toString());
echo('----');
echo('ring.fromString("(1+x*y)/(x-y)")');
